//! Rosenbrock Annealing Visualization
//!
//! Visualizes simulated annealing on the Rosenbrock "banana" function.
//! Easy to find the valley, hard to find the minimum at (1,1).
//! Watch particles slide along the narrow curved valley.
//!
//! Run with: cargo run --release --features "viz gpu" --bin rosenbrock-annealing
...
// Annealing schedule
const TOTAL_STEPS: u32 = 4000; // Rosenbrock is harder, need more steps
const T_START: f32 = 5.0;
const T_END: f32 = 0.001;

// Domain (shifted to show the minimum at (1,1))
const DOMAIN_MIN: f32 = -2.0;
const DOMAIN_MAX: f32 = 3.0;
const DOMAIN_SIZE: f32 = DOMAIN_MAX - DOMAIN_MIN;

fn main() -> iced::Result {
    iced::application(App::new, update, view)
        .subscription(subscription)
        .title("Rosenbrock Annealing - Following the Banana Valley")
        .run()
}

// Rosenbrock function for rendering heatmap
...
impl App {
    fn new() -> Self {
        let mut system = ThermodynamicSystem::with_loss_function(
            PARTICLE_COUNT, DIM, T_START, LossFunction::Rosenbrock
        );
    ...
    match message {
        Message::Tick => {
            ...
            for _ in 0..app.speed {
                ...
                let dt = if app.temperature > 0.1 { 0.005 }
                        else if app.temperature > 0.01 { 0.003 }
                        else { 0.001 };
...
fn view(app: &App) -> Element<'_, Message> {
    ...
    let stats = format!(
        "Min Loss: {:.4} | Mean: {:.2} | Best: ({:.3}, {:.3}) | Global optimum: (1, 1)",
...
impl canvas::Program<Message> for App {
    ...
    fn draw(
        ...
        let heatmap = self.heatmap_cache.draw(renderer, bounds.size(), |frame| {
            ...
            let resolution = 100; // Higher resolution for the banana shape
            ...
            for i in 0..resolution {
                for j in 0..resolution {
                    ...
                    // Log scale for better visualization (Rosenbrock has huge range)
                    let log_loss = (loss + 1.0).ln();
                    let t = (log_loss / 10.0).clamp(0.0, 1.0);

                    // Banana valley colors: dark blue for valley, yellow/red for hills
                    let color = if t < 0.1 {
                        // Valley - dark blue to green
                        Color::from_rgb(0.0, t * 5.0, 0.3 - t * 2.0)
                    } else if t < 0.4 {
                        // Moderate slopes - yellow
                        Color::from_rgb(t * 2.0, t * 1.5, 0.0)
                    } else {
                        // High loss - red/orange
                        Color::from_rgb(0.8, 0.4 - t * 0.3, 0.0)
                    };
            ...
            // Draw the parabola y = x^2 (the valley floor)
            let valley_color = Color::from_rgba(1.0, 1.0, 1.0, 0.2);
            for i in 0..100 {
                let x = DOMAIN_MIN + (i as f32 / 100.0) * DOMAIN_SIZE;
                let y = x * x;
                if y >= DOMAIN_MIN && y <= DOMAIN_MAX {
                    let px = ((x - DOMAIN_MIN) / DOMAIN_SIZE) * size.width;
                    let py = ((y - DOMAIN_MIN) / DOMAIN_SIZE) * size.height;
                    let dot = canvas::Path::circle(Point::new(px, py), 1.5);
                    frame.fill(&dot, valley_color);
                }
            }

            // Mark global optimum at (1, 1)
            let ox = ((1.0 - DOMAIN_MIN) / DOMAIN_SIZE) * size.width;
            let oy = ((1.0 - DOMAIN_MIN) / DOMAIN_SIZE) * size.height;
        ...
        let particles_geo = self.cache.draw(renderer, bounds.size(), |frame| {
            ...
            for p in &self.particles {
                ...
                // Color based on energy (log scale)
                let log_e = (p.energy + 1.0).ln();
                let t = (log_e / 8.0).clamp(0.0, 1.0);
